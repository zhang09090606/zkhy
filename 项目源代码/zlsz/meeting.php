...
<head>
	...
	<script src="js/jquery.3.2.1.min.js"></script>
		...
		<script>
			$(function(){
				$("[id=delsignup]").click(function(){
				 if(confirm('确定要取消报名吗')==true){
					id=$(this).next().val();
					$.ajax({
						type: "POST",
						url: "php/meeting_del.php",
						data: {"id": id,"state":"signup"},
						success: function (data) {
							alert("取消报名成功");
				...
				$("[id=delmeeting]").click(function(){
				 if(confirm('确定要取消会议吗')==true){
					id=$(this).next().val();
					$.ajax({
						type: "POST",
						url: "php/meeting_del.php",
						data: {"id": id,"state":"meeting"},
						success: function (data) {
							alert("取消会议成功");
									...
									<div class="col-12 d-flex align-items-center">
										<div class="numbers col-12">
											...
											<div style="float:left;margin-left: 20px;margin-top: 10px;">
												<?php
													if($arr[0]['type']=='private'){
														if($arr0[$i]['state']=='false'){
												?>
													<p>报名审核状态：<font color="red">等待通过</font></p>
													<?php
														}else{
													?>	
												<p>报名审核状态：<font color="green">通过报名</font></p>
												<?php
													}
													}
												?>
												...
												?>	
												<center><button id="delmeeting" class="btn btn-warning" data-loading-text="Loading..." type="button"> 取消会议
								      				  </button>
								      				  <input value="<?php echo $arr[0]['id']?>" hidden="">
												</center>
												<?php
													}else{
												?>
												<center><button id="delsignup" class="btn btn-warning" data-loading-text="delsignup" type="button"> 取消报名
								      				  </button>
								      				   <input value="<?php echo $arr0[$i]['id']?>" hidden="">
												</center>
												<?php
													}
												?>
