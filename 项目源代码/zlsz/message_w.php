...
$arr = $Db->get_all( "message","user='$name' and state='false'");
$list=$Db->get_all("message","type!='审核'");
for($i=0;$i<sizeof($list);$i++){
	$id=$list[$i]['id'];
	$data = array(
		'state' =>"true"
	);
		$Db->updatedata( "message", $data, "id='$id'" );
}
?>
<html>
		...
		<script>
			$(function(){
				...
				$("#del").click(function(){
					...
					$.ajax({
						...
						success: function (data) {
							...
							$(location).attr('href', 'message_w.php');
						}
					});
					}
				})
				$("#success").click(function(){
					...
					sendpeo=$(this).next().next().next().val();
					$.ajax({
						type: "POST",
						url: "php/agree.php",
						data: {"id": id,"sendpeo":sendpeo},
						success: function (data) {
							alert("通过成功");
				...
				$("#refuse").click(function(){
					...
					if(confirm('确定要不通过吗')==true){
					id=$(this).next().val();
					sendpeo=$(this).next().next().val();
					$.ajax({
						type: "POST",
						url: "php/refuse.php",
						data: {"id": id,"sendpeo":sendpeo},
					...
					<ul class="nav">
						...
						<li class="nav-item active">
							<a href="message_w.php">
								...
								<p>未读消息</p>
								<span class="badge badge-danger" <?php if(sizeof($arr)==0){echo "hidden";}?>><?php echo sizeof($arr)?></span>
													...
													<div class="col-12 d-flex align-items-center">
														<div class="numbers col-12">
															...
															<div style="float:left;margin-left: 20px;margin-top: 10px;">
																...
																<center>
																	<button id="success" class="btn btn-success" data-loading-text="Loading..." type="button" <?php if($arr[$i]['type']!='审核'){echo 'hidden';}?>> 通过报名
																	</button>&nbsp;&nbsp;
																	<button id="refuse" class="btn btn-warning" data-loading-text="Loading..." type="button" <?php if($arr[$i]['type']!='审核'){echo 'hidden';}?>> 拒绝通过
																	 </button><input type="text" value="<?php echo $arr[$i]['meetingid']?>" hidden="">
																	 <input type="text" value="<?php echo $arr[$i]['send_peo']?>" hidden="">&nbsp;&nbsp;<button id="del" class="btn btn-danger" data-loading-text="Loading..." type="button" <?php if($arr[$i]['type']=='审核'){echo 'hidden';}?>> 删除信息
																  </button></center>
